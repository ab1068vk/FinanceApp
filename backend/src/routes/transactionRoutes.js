...
const createRules = [
  ...
  // API contract: clients submit and receive transaction amounts as decimal major units
  // (for example, 12.34 means $12.34). Values must be finite with max 2 decimals.
  body('amount')
    .isFloat({ gt: 0, max: MAX_TRANSACTION_AMOUNT })
    .withMessage(`amount must be greater than 0 and no more than ${MAX_TRANSACTION_AMOUNT}`)
    .bail()
    .matches(/^\d+(\.\d{1,2})?$/)
